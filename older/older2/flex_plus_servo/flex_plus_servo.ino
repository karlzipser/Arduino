...
float previous_servo_position = 90;
float s = 0.95;
void loop() 
{ 
  int flexposition;    // Input value from the analog pin.
  float servoposition;   // Output value to the servo.
  ...
  //servoposition = map(flexposition, 550,300, 0, 180);
  servoposition = abs(440-flexposition);
  servoposition = map(servoposition, 0,180, 0, 180);
  servoposition = (1-s)*servoposition+s*previous_servo_position;
  previous_servo_position = servoposition;
  servoposition = constrain(servoposition, 0, 180);

  // Now we'll command the servo to move to that position:

  servo1.write(int(servoposition));
  ...
  delay(1);  // wait 20ms between servo updates
} 
