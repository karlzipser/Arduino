...
volatile int state = STATE_HUMAN_FULL_CONTROL_CENTER;
...
void button_interrupt_service_routine(void) {
  ...
  if (dt>BUTTON_MIN && dt<BUTTON_MAX) {
    ...
    if (abs(button_pwm_value-BUTTON_A)<BUTTON_DELTA) {
      ...
      if (state != STATE_HUMAN_FULL_CONTROL_LEFT) {
        previous_state = state;
        state = STATE_HUMAN_FULL_CONTROL_LEFT;
      ...
      if (state == STATE_ERROR) return;
      if (state != STATE_HUMAN_FULL_CONTROL_CENTER) {
        previous_state = state;
        state = STATE_HUMAN_FULL_CONTROL_CENTER;
        state_transition_time_ms = m/1000.0;
      }
    }
    else if (abs(button_pwm_value-BUTTON_C)<BUTTON_DELTA) {
      if (state == STATE_ERROR) return;
      if (state != STATE_HUMAN_FULL_CONTROL_RIGHT) {
        previous_state = state;
        state = STATE_HUMAN_FULL_CONTROL_RIGHT;
        state_transition_time_ms = m/1000.0;
      }
    }   
...
void servo_interrupt_service_routine(void) {
  ...
  if (dt>SERVO_MIN && dt<SERVO_MAX) {
    servo_pwm_value = dt;
    if (state == STATE_HUMAN_FULL_CONTROL_LEFT || state == STATE_HUMAN_FULL_CONTROL_CENTER || state == STATE_HUMAN_FULL_CONTROL_RIGHT){
